<form method="POST" class="col-12 ajaxForm" action="{{ route('student.update', $student->id) }}" id = "student_update_form">
    @csrf
    @method('PATCH')
    <div class="col-12">
            ...
            <div class="col-md-9">
                <select id="parent_id" name="parent_id" class="form-control" required >
                    ...
                    @foreach (\App\User::where('school_id', school_id())->where('role', 'parent')->get() as $parent)
                        <option value="{{ $parent->id }}" @if($parent->id == $student->parent_id) selected @endif>{{ $parent->name }}</option>
                    @endforeach
                </select>
            </div>
        </div>
